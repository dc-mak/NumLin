...
expression are translated into BLAS/ LAPACK calls via pattern-matching,
outlined in Figure~\ref{fig:lang_matexp}. Type checking is mostly standard for
a linearly typed language, with the exception of fractional permission
inference.  Because all functions must have their argument types explicitly
annotated, inferring the correct fraction at a call-site is simply a matter of
unification.
...
\caption{Pattern-matching on matrix expressions.}\label{fig:lang_matexp}
\end{figure}
