        ...
        \case{%
            ...
            \begin{proof}
                ...
                \step{}{If it is not a value, we are done. If it is $({\sigma_e}_f, v)
                    ...
                    \gamma(\delta(e')) [x/v]) \in \C{k-j-1}{\theta(t')}$.}}

                \step{}{\define{$\gamma_{e'} (y) = v$ if $ y=x $ and $\gamma(y)$ if $y \in \dom(\Gamma')$.}
                    Thus, $(\sigma_{e'}, \gamma_{e'}) \in \den{L}{k}{\Gamma', x : t} \subseteq
                    \den{L}{k-j-1}{\Gamma', x : t}$.}

                \step{}{Instantiate induction hypothesis $\den{}{}{\Theta; \Delta; \Gamma', x : t
                    \vdash e' : t'}$, with $\theta, k-j-1, \delta, \gamma_{e'}, \sigma_{e'}$ to
                    conclude $(\sigma_{e'}, \gamma_{e'}(\delta(e'))) \in \C{k-j-1}{\theta(t')}$.}
