...
\highl{x < 0 || x = 0}, it does not). Any linear values used by the conditional
...
simplifications are:
\begin{itemize}
    \item the kernel has a fixed length (of 3), as such, only the value of
        \highl{write[i-1]} prior to modification in the previous iteration
        needs to be carried forward using \highl{x0}
...
\emph{The most pertinent aspect of \lang's type system are the types of its
primitives.} While the types of operations such as \texttt{get} and
\texttt{set} might be borderline obvious, they types of BLAS/LAPACK routines
become an \emph{incredibly useful, automated check for using the API correctly.}

Figure~\ref{fig:lang_lin_reg} shows a \emph{non-recursive} function declaration (the
return type is inferred). Since we would like to be able to use a function like
\texttt{lin\_reg} more than once, it is marked with a \texttt{!} annotation
(which also ensure it captures no linear values from the surrounding
environment).

We wish to compute $\hat\beta = \mathbf{(X^T X)^{-1} X^T y}$. First, we extract
the dimensions of matrix \texttt{x}. Then, we say we would like a new matrix, of
dimension $m \times 1$, which contains the result of $\mathbf{X^T y}$. The line
\highl{let xy <- new (m,1) [| x^T * y |]} is syntactic sugar for first creating
a new matrix \highl{let xy = matrix m 1 in} and then storing the result of
\highl{(x, true (* transp'd *))} multiplied with \highl{(y, false (* not
transp'd *))} into \texttt{xy}: \highl{let (xy, (x, y)) = gemm 1.  _ (x,
true) _ (y, false) 0. xy}, appropriately re-binding the identifiers.  The line
\highl{let x_T_x <- new (m,m) [| x^T * x |]}, works similarly, but uses a more
appropriate, \highl{syrk true 1. x 0. x_T_x} BLAS call.

By using some simple pattern-matching and syntactic sugar, we can:
\begin{itemize}
    \item write normal-looking, apparently non-linear code
    \item use matrix expressions directly and have an appropriate, efficient
        call to a BLAS/LAPACK routine inserted
...
the form \highl{let <id> <- [| beta * c + alpha * a * b |]} are desugared to
\texttt{gemm} calls. Primitives like \highl{transpose: 'x. 'x mat --o 'x mat *
z mat} and \highl{eye: !int --o z mat} allocate new matrices;
\texttt{transpose} returns the transpose of a given matrix and \texttt{eye}
...
the value stored in \texttt{tmp\_5\_5} after the call to \texttt{gesv}. However,
we can use the memory much later to store \texttt{answer} with \highl{let
answer <- [| 0. * tmp_5_5 + q_inv_u * inv_u_T |]}. Again, thanks to linearity,
the identifiers \texttt{q}, \texttt{tmp\_5\_5} are out of scope by the time
\texttt{answer} is bound. Although during execution, all three refer to the
same piece of memory, logically they represent different values throughout the
computation.
...
\begin{figure}[t]
    ...
    \caption{Kalman filter: see Figure~\ref{fig:maths_kalman} for the
        equations this code implements.}\label{fig:lang_kalman}
\end{figure}
...
(equations in Figure~\ref{fig:maths_kalman}). A few new features and techniques
...
\begin{itemize}
    ...
    \item \texttt{sym} annotations in matrix expressions: when this is used, a 
        ...
        inserted (instead of one to \texttt{gemm})
