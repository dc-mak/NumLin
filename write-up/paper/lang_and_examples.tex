...
get very difficult to ensure memory is not being re-used incorrectly or re-used
enough whilst also enusring the aliasing assumptions for the BLAS/LAPACK
routine calls are guaranteed. However, using \lang\ makes incorrect aliasing
and memory re-use impossible. Additionally, \lang\ makes it very easy to spot
where memory is not being re-used enough \emph{precisely because} of its
explicit handling of matrix allocations, copies and frees.
