...
\begin{proof}
    ...
    \step{}{%
        \case{%
            ...
            \suffices{$(\sigma, \theta(\delta(\gamma(\ottkw{fix} (g, x : t, e : t'))))) \in
                \V{k}{\theta(t \multimap t')}$, by \ref{subsetVC}.}

            \assume{Arbitrary $j \leq k$, $(\varsigma_v, v) \in \V{j}{\theta(t)}$ such that
                $\varsigma \star \varsigma_v$ is defined.}

            \pflet{$\tilde{e} \equiv \theta(\delta(\gamma(e)))$.}

            \suffices{$(\varsigma \star \varsigma_v,\ottkw{fix} (g, x : t,
                \tilde{e} : t')\ v) \in \C{j}{\theta(t')}$.}

            \suffices{$(\varsigma \star \varsigma_v, \tilde{e}\,
                [x / v]\, [g /\ottkw{fix} (g, x : t, \tilde{e} : t')]) \in
                \C{j-1}{\theta(t')}$ by \ref{stepInC}.}
    ...
    \begin{proof}
        ...
        \step{}{Instantiate this with $j-1$.}
