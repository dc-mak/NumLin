...
\begin{proof}
    ...
    \step{}{$v'$ is of the correct syntactic form (lambda or fixpoint) by assumption.}
    \step{}{\assume{arbitrary $j' \leq j$ and $(\varsigma_v, v) \in \V{j'}{t}$
        such that $\varsigma_{v'} \star \varsigma_v$ is defined.}}
    \step{}{\suffices{to show $(\varsigma_{v'} \star \varsigma_v, v' v) \in \C{j'}{t'}$.}}
    \step{}{\suffices{to show $(\varsigma_{v'} \star \varsigma_v, v' v) \in
        \C{j}{t'}$ by $\C{j}{} \subseteq \C{j'}{}$.}}
    \step{}{Instantiate $(\varsigma_{v'}, v') \in \V{k}{t \multimap t'}$ with
        $j$ and $(\varsigma_v, v) \in \V{j}{t} $ ??? by induction on $t$.}
