...
\texttt{get} and \texttt{set} might be borderline obvious, the types of
...
%
        \ottkw{symm} &:& \; !\Bool \multimap \; !\Elt \multimap \; {'x}. \:
            ...
            \multimap \texttt{z}\ \Mat \multimap
            \\ && ( {'x}\ \Mat \otimes {'y}\ \Mat ) \otimes \texttt{z}\ \Mat \\
%
        \ottkw{gemm} &:& \; !\Elt \multimap \; {'x}. \:  {'x}\ \Mat \: \otimes \;
            !\Bool \multimap \; {'y}. \:  {'y}\ \Mat \: \otimes \; !\Bool
            \multimap \; !\Elt \multimap
            \\ && \texttt{z}\ \Mat \multimap ( {'x}\ \Mat \otimes {'y}\ \Mat )
            \otimes \texttt{z}\ \Mat \\
%
        \ottkw{gesv} &:& \texttt{z}\ \Mat \multimap \texttt{z}\ \Mat \multimap
            \texttt{z}\ \Mat \otimes \texttt{z}\ \Mat \\
%
        \ottkw{posv} &:& \texttt{z}\ \Mat \multimap \texttt{z}\ \Mat \multimap
            \texttt{z}\ \Mat \otimes \texttt{z}\ \Mat \\
%
        \ottkw{posv\_flip} &:& \texttt{z}\ \Mat \multimap \texttt{z}\ \Mat
            \multimap \texttt{z}\ \Mat \otimes \texttt{z}\ \Mat \\
%
        \ottkw{potrs} &:& {'x}. \:  {'x}\ \Mat \multimap \texttt{z}\ \Mat
            \multimap {'x}\ \Mat \otimes \texttt{z}\ \Mat \\
%
        \ottkw{syrk} &:& \; !\Bool \multimap \; !\Elt \multimap \; {'x}. \:
            {'x}\ \Mat \multimap \; !\Elt \multimap \texttt{z}\ \Mat \multimap \;
            {'x}\ \Mat \otimes \texttt{z}\ \Mat \\
%
    \end{array} \]
    \end{center}
...
need full \texttt{z} permissions; the first would be fraction-polymorphic.
