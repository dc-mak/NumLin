...
for \highl{gemm}). The check being performed is a simple sanity check that the
two aliasing pointers passed to \highl{unshare} point to the same array/matrix.
...
If the programmer did try to mutate or free \highl{row}, then they would get a
...
with exactly the same fractional permission as the \highl{row} (which can only
be \highl{row}).  This is necessary because of linearity: for the caller, the
...
An example of this consuming and rebinding is in \highl{let (row, !x1) =
row[i]}. Indexing is implemented as a primitive \texttt{get: 'x. 'x arr --o
...
\emph{automatically inferred at call sites}, \texttt{row[i] == get \_ row i}
