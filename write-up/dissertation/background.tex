...
\highl{`!'}s found here and there (they are annotations to show that we can
use a value more than once). In Figure~\ref{fig:ltfla_sumarray}, we see
\highl{row} has type (\highl{'x arr}). More detailed explanations of
\emph{what and why} will be given in Chapter~\ref{chap:impl}, but for now, it
is enough to know it means we can only \emph{read} from \highl{row}, and not
write to it. If we did try to write to or free \highl{row}, we would get a
...
assume `simple\_dgemm' has type \highl{'x . 'x mat --o 'y .  'y mat --o z mat
...
To solve this, we use a special primitive called \highl{share} to produce
...
so, we must first \highl{unshare} any read-only aliases that exist, to get back
...
let () = free a in c
    \end{minted}
    \caption{Squaring a matrix in LT4LA (assuming \highl{c} is initially
        0).}\label{fig:ltfla_square}
\end{figure}

If we tried to free one of the read-only aliases before or instead of
\highl{unshare}-ing them, then we would get the error shown in
Figure~\ref{fig:ltfla_free}. Briefly, a \highl{z arr} is a read-write array,
everything else (\highl{'x arr} or \highl{_ s arr}) is read-only. The types of
\highl{free}, \highl{share} and \highl{unshare} (see
...
if we \emph{forgot} to \highl{free a}, we would also get \texttt{Variable a not
used} error.
...
\highl{share} prevents this as well -- \highl{let (a11, a12) = share _ a1 in
