...
Operationally, $\emph{Heap} \subseteq \emph{Loc} \times \emph{Permission}
\times \emph{Matrix} $, denoted with a $\sigma$.\\
...
\\
    \C{k}{ t } &= \{ (\varsigma_s, e) \mid \forall \, j \leq k, \varsigma_r.\ \varsigma_s \star \varsigma_r \textrm{ defined } \Rightarrow \langle \sigma_s \uplus \sigma_r, e \rangle \rightarrow^j \ottkw{err}\ \vee \exists \sigma_f, e'.\\
               & \qquad \qquad \langle \sigma_s \uplus \sigma_r, e \rangle \rightarrow^j \langle \sigma_f \uplus \sigma_r, e' \rangle \wedge ( e' \textrm { is a value } \Rightarrow ( \varsigma_f \star \varsigma_r, e' ) \in \V{k-j}{t} ) \} \\
\\
    \den{I}{k}{ \cdot } \theta &= \{ [] \} \\
\\
