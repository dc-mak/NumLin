...
\\
    \V{k}{ \forall fc.\  t } &= \{ (\varsigma, \ottkw{fun}\, fc \rightarrow \, e) \mid \forall f.\ (\varsigma, (\ottkw{fun}\, fc \rightarrow \, e)\, [ f ]) \in \C{k}{ t [ fc / f ] } \} \\
\\
...
\den{}{k}{ \Theta; \Delta ; \Gamma \vdash e : t } &= \forall \theta, \delta, \gamma, \sigma.\ \Theta = \dom(\theta) \wedge (\varsigma, \gamma) \in \den{L}{k}{ \Gamma }\theta \wedge \delta \in \den{I}{k}{ \Delta }\theta \Rightarrow \\
                                                 & \qquad \qquad (\varsigma, \theta(\gamma(\delta(e)))) \in \C{k}{ \theta(t) }
\end{align*}
